...
var SlimPicker = new Class({
	...
	options: {
		...
		inputEleClass: 'DatePicker',
		cssClassPrefix: 'gxComDatePicker',
		containerClass: 'Container',      // This will always start at the top left of the input's location.
		calendarClass: 'Calendar',        // Use this to alter the placement of the calendar in the CSS.
		hoverClass: 'Hover',              // If using the keyboard, this gets moved around the calendar by arrow keys.
		selectedClass: 'Selected',        // The date picked up from what was in the input field.
		todayClass: 'Today',              // Always just on today. The sp_selected usually overrides this.
		emptyClass: 'Empty',              // Placed on the <td> of a date with no day in it.
		dayClass: 'Day',                  // Placed on the <td> with a day in it.
		monthClass: 'Month',              // On the dropdown for month.
		yearClass: 'Year',                // On the dropdown for year.
		backArrowClass: 'BackArrow',      // Placed on the <a> with the back arrow
		nextArrowClass: 'NextArrow',      // Placed on the <a> with the next arrow

		fadeDuration: 200,                // How fast the calendar fades in and out.
		hideDelay: 1500,                  // How long to wait to close the calendar after the mouse leaves.
		...
		format: '%Y-%m-%d',                                            // Date input and output format (see Mootools Date.format()) example('%d.%m.%Y %H:%M:%S'),
		yearStart: 5,                                                  // Default starting year for dropdown options is 5 years ago
	...
	initialize: function(el, options) {
		...
		this.options.yearStart = (new Date().getFullYear() - this.options.yearStart);
		var css = this.options.cssClassPrefix;
		this.options.containerClass = css + this.options.containerClass;
		this.options.calendarClass = css + this.options.calendarClass;
		this.options.hoverClass = css + this.options.hoverClass;
		this.options.selectedClass = css + this.options.selectedClass;
		this.options.todayClass = css + this.options.todayClass;
		this.options.emptyClass = css + this.options.emptyClass;
		this.options.dayClass = css + this.options.dayClass;
		this.options.monthClass = css + this.options.monthClass;
		this.options.yearClass = css + this.options.yearClass;
		this.options.backArrowClass = css + this.options.backArrowClass;
		this.options.nextArrowClass = css + this.options.nextArrowClass;

		// Saving the input field
		this.input = $(el);

		if ( !this.input.hasClass(this.options.inputEleClass) )
			this.input.addClass(this.options.inputEleClass)
		...
		//this.doc.addEvent((Browser.ie || Browser.safari || Browser.chrome) ? 'keydown' : 'keypress', this.checkKeys.bind(this));
		this.doc.addEvent('keydown', this.checkKeys.bind(this));
	...
	show: function() {
		if (this.open) return ( this.options.keepEditable ? undefined : true );
		this.open = true;
...
//					this.moveFocus(e.shift);
	...
	useSelection: function(el) {
		...
		var currentDate = new Date().parse(this.input.get('value'));
		var hour = currentDate.format('%H');
		var min  = currentDate.format('%M');
		var sec  = currentDate.format('%S');
	...
	draw: function() {
		...
		var uri;
		var cssClassAdd;
		var daysInTodaysMonth = this.options.daysInMonth[this.calendarMonth];
		// Keeping track of row for hoveredDay purposes
		var row = 0;
		while (currentDay <= daysInTodaysMonth){
			row += 1;
			str += '<tr>';
			for (i = 0; i < 7; i++){
				if ((currentDay <= daysInTodaysMonth) && (currentDay > 0)){
					var drawingDate = new Date(this.calendarYear, this.calendarMonth, currentDay);
					cssClassAdd = '';
				} else {
					drawingDay = currentDay;
					// If we got 5 days of last month inside this view current day = -4
					// this works fine in this while loop cause 4 + 0 = 5
					// but for correct calculating we need the -5
					if ( currentDay <= 0 )
						drawingDay--;

					var drawingDate = this.getDateOfNeighborsMonth(this.calendarYear, this.calendarMonth, drawingDay);
					cssClassAdd = ' ' + this.options.emptyClass;
				}

				drawingDay = drawingDate.getDate();
				drawingMonth = drawingDate.getMonth();
				drawingYear = drawingDate.getFullYear();

				str += '<td><a href="#' + drawingYear + '|' + drawingMonth + '|' + drawingDay + '" class="' + this.options.dayClass + cssClassAdd;
				// Show the currently selected day
				if ( this.input.value != '' && (drawingDay == this.currentDay) && (drawingMonth == this.currentMonth) && (drawingYear == this.currentYear) ) {
					str += ' ' + this.options.selectedClass;
					this.hoverRow = row;
					this.hoverCol = i;
				}
				// Show today
				if ( (drawingDay == this.nowDay) && (drawingMonth == this.nowMonth) && (drawingYear == this.nowYear) ) {
	...
	getDateOfNeighborsMonth: function (year, month, day) {
		var daysInMonth = this.options.daysInMonth[month];
		if ( day > daysInMonth ) {
			day = day - daysInMonth;
			month++;
		} else if ( day <= 0 ) {
	...
	addMonthYearDropdowns: function () {
		var str = '<thead><tr><td colspan="7">';
		var show;
		if (this.options.showMonth) {
			show = '';
			if ( parseInt(this.calendarYear) == this.options.yearStart && this.calendarMonth == 0 )
				show = 'style="display:none;"';

			str = '<thead><tr><td><a href="#" '+show+' class="'+this.options.cssClassPrefix+'NavArrows ' + this.options.backArrowClass + '"></a></td><td colspan="5">';
		}
		...
		if (this.options.showMonth) {
			show = '';
			if ( parseInt(this.calendarYear) == (this.options.yearStart + this.options.yearRange) && this.calendarMonth == 11)
				show = 'style="display:none;"';

			str += '</td><td><a href="#" '+show+' class="'+this.options.cssClassPrefix+'NavArrows ' + this.options.nextArrowClass + '"></a></td></tr></thead>';
		}
		else
			str += '</td></tr></thead>';
	...
	addCalendarEvents: function() {
		...
		this.tbody.addEvent('click', this.calendarClick.bind(this));
		...
		this.calendar.getElements('.'+this.options.cssClassPrefix+'NavArrows' ).addEvent('click', this.changeMonth.bind(this) );
	},
	...
	changeMonth: function (e) {
		...
		if( e.target.hasClass(this.options.nextArrowClass) )
